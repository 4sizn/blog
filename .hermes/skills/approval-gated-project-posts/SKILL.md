...
프로젝트 기록에 작가가 제공한 개인적 출발점이 있으면, 그것을 문단의 중심에 둔다. `친구의 요청을 제품 경험으로 번역했다`, `존재감을 만들었다`, `장면을 구현했다` 같은 정리된 기획서 문장은 글을 멀어 보이게 만든다. 독자가 보고 싶은 것은 기획 요약이 아니라 **누가 무엇을 말했고, 작성자가 왜 그 부분을 붙잡았는지**다.

1. 첫 문단은 제품 정의보다 실제 시작점으로 연다. 예: `20년 지기 친구가 어느 날 바탕화면에 둘 만한 걸 만들어 달라고 했다.`
2. 작가가 제공한 호칭, 짧은 대사, 부탁, 관찰은 그대로 살린다. 단, 제공되지 않은 대화·사건·감정은 꾸며내지 않는다.
3. 기능마다 기능명세를 나열하지 말고, `왜 여러 마리로 했는지`, `왜 밥주기를 넣었는지`, `왜 화면을 덜 차지하게 했는지`처럼 한두 개의 실제 판단을 1인칭으로 풀어 쓴다.
4. 단락은 고르게 맞추지 않는다. 짧은 문장과 생각이 이어지는 문장을 섞고, 결론을 교훈이나 홍보 문구로 부풀리지 않는다.
5. 원고를 소리 내어 읽었을 때 “프로젝트 소개서”처럼 들리면 다시 쓴다. 아래 표현을 우선 제거하거나 평이한 1인칭 문장으로 바꾼다.
   - `제품 경험으로 번역했다`, `존재감`, `경험을 결정한다`, `출발점이 됐다`, `목표로 했다`
   - `단순히 A가 아니라 B`, `이 세 가지`, 과도하게 대칭적인 3단 나열
   - 기능을 설명한 직후의 일반론적 의미 부여

사람다운 문체는 사실을 덜 엄밀하게 쓰라는 뜻이 아니다. 사실 경계는 유지하되, 작가가 실제로 제공한 관계·관찰·판단만으로 글의 온도를 만든다. 개인적 장면이 하나도 제공되지 않았다면 억지로 회고체를 만들지 말고, 차분한 제품 기록으로 쓴다.
