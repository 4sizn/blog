...
| placeholder | 값 | 비었을 때 |
|-------------|-----|----------|
| `{{INTRO_TODO}}` | 도입부 자리 TODO 마커 | 항상 삽입 (도입부는 필수 섹션이며 자동으로 채울 수 없다) |
...
| DD-13 | 릴리즈 본문의 **검증 로그 섹션**(`Verification`·`Testing`·`Test plan`·검증·테스트)은 파싱에서 건너뛴다 | ① 전부 `Improvements` 로 흡수 ② 사람이 지우도록 그대로 둔다 | 구현 중 실제로 터진 문제다. `ai-config-monitor` v1.2.1 본문의 `## Verification` 항목이 `Improvements` 로 흡수되면서 `bun run build passed` 와 **로컬 절대 경로**(`/Users/…/blender-test/.codex/mcp.json`)가 초안 본문에 들어갔다. 사람이 지우는 것에 기대면 한 번 놓치는 순간 개인 경로가 게시된다. 원문은 `SOURCE` 주석에 남으므로 근거는 잃지 않는다 |
...
| 날짜 | 변경 | 사유 |
|------|------|------|
| 2026-08-11 | §5-1 신규 placeholder 8개 → **9개** (`{{INTRO_TODO}}` 추가) | 도입부는 필수 섹션인데 자동으로 채울 수 없다. 마커가 없으면 초안이 필수 섹션 없이 나온다 (W-01 구현 중 발견) |
| 2026-08-11 | DD-13 추가 (검증 로그 섹션 파싱 제외) | 초안에 로컬 절대 경로가 실려 나오는 것을 확인했다 (W-03 구현 중 발견) |
