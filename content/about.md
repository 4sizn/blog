---
title: "About"
description: "웹 개발자 신희석입니다. React 기반 UI와 실시간 메시지 프로토콜 기반 클라이언트 기능을 만들어 왔습니다."
draft: false
lang: ko
---
...
#### 메타버스 서비스 · 2022.08 - 2025.07

- 실시간 메시지 처리 기반 UI 컨텐츠 개발 (채팅, Web UI)
- Service Core Logic 개발 (Network, Cache, Auth)
- 프로젝트 환경 및 구조 설계

#### 리모트 미팅 - 원격 미팅 서비스 · 2021.06 - 2022.03

- 레거시 프로젝트 리뉴얼 전환 (React, Recoil)

#### 리모트 세미나 - 웨비나 서비스 · 2019.04 - 2021.04

- 신규 웨비나 서비스 개발 (React, Redux)
- 어드민 사이트 신규 개발 (React, Rematch)

#### 모비즌 - 스마트폰 미러링 서비스 · 2018.12 - 2020.12

- 페이지 유지보수 및 결제 페이지 제작
- WebRTC를 이용한 POC 프로젝트

### 드래곤플라이 · ARVR팀 / 사원

**2016.08 - 2017.05**

#### 스페셜 포스 VR - FPS 게임 · 2017.03 - 2017.09
...
**2021.06 - 2022.03** · 화상 미팅 서비스
`TypeScript` `React` `Recoil` `WebRTC` `styled-components`

- 서비스 전면 리뉴얼을 위해 레거시 jQuery UI를 React와 Recoil 기반으로 전환했습니다.
- 공용 컴포넌트, 사용자 초대와 데이터 정규화, Video 컴포넌트 및 기능을 개발했습니다.
- Recoil 상태 관리에서 서비스 트랜잭션과 렌더링 최적화를 고려했습니다.

### 리모트 세미나

**2019.04 - 2021.04** · 웹 브라우저 기반 웨비나 서비스
`TypeScript` `React` `Redux` `Redux-Saga` `WebRTC` `YouTube`

- 웨비나 서비스와 관리자 페이지를 신규 개발했습니다.
- 세미나 양식, 설문, 위지위그 편집기 커스터마이징, 파일 다운로드, 카운트다운과 YouTube 커스텀 플레이어를 구현했습니다.
- BroadcastChannel을 활용해 브라우저 창 사이의 실시간 데이터 동기화를 구현하고, Normalizr로 데이터를 정규화했습니다.

### 모비즌

**2018.12 - 2020.12** · PC/모바일 미러링 서비스
`JavaScript` `jQuery` `JSP` `CSS` `WebRTC`

- 휴대폰과 PC 간 원격 제어 및 미러링 서비스의 유지보수와 결제 페이지 제작을 담당했습니다.
- WebRTC 기반 POC인 Mobizen Party를 개발해, 최대 4개의 PC/모바일 화면을 연결하고 공유하는 경험을 구현했습니다.

### SpecialForce ARVR

**2017.03 - 2017.09** · VR FPS 게임
`Unity` `C#` `FSM` `AI` `VR`

- Unity 기반 VR FPS 게임 제작에 참여했습니다.
- FSM(Finite State Machine) 기반 AI와 UI, 튜토리얼을 개발했습니다.
- 3D 렌더링, Unity 및 VR 개발 역량을 쌓고, VR 환경에서의 사용자 친화적 UI와 QA를 함께 고려했습니다.
...
다만 지금까지 그래 왔듯 새로운 기술을 받아들이고 응용해서, 사용자에게 도움이 되는 것을 만들려 합니다.

결국 어느 분야든 목표는 같다고 생각합니다. 쓰는 사람이 편해지는 것,
그래서 서비스가 좋은 인상으로 남는 것. 제 자리에서 할 수 있는 일은 서비스의 질을 높여 그 편리함을 만드는 것입니다.

사용자 데이터를 읽고 거기서 새로운 서비스를 끌어내는, 고객을 먼저 생각하는 개발자가 되고 싶습니다.

## Contact

가장 확실한 연락 방법은 이메일입니다 — [4sizn@naver.com](mailto:4sizn@naver.com)

- GitHub: [github.com/4sizn](https://github.com/4sizn)
- LinkedIn: [linkedin.com/in/4sizn](https://www.linkedin.com/in/4sizn)
